...
function renderMarkdownReport(b) {
  ...
  if (b.runtimeEvents.length) {
    ...
    L.push('');
    L.push('| Time | Status | Severity | Source | Action | Detail |');
    L.push('| --- | --- | --- | --- | --- | --- |');
    for (const l of b.runtimeEvents.slice(0, 300)) {
      L.push(`| ${l.timestamp} | ${l.status} | ${l.severity} | \`${l.sourcePackage}\` | ${l.action} | ${(l.details || '').replace(/\|/g, '\\|').slice(0, 160)} |`);
    }
    L.push('');
  }
  if (b.scans.length) {
...
${logRows ? `<h2>Runtime telemetry</h2><table><thead><tr><th>Time</th><th>Status</th><th>Source</th><th>Action</th><th>Detail</th></tr></thead><tbody>${logRows}</tbody></table>` : ''}
...
function createApi(ctx) {
  const { db, config, getReport, runScan, project, autoscan } = ctx;
  ...
  router.get('/forensics/report', async (req, res) => {
    ...
    if (format === 'csv') {
      const rows = [
        ...
        ...logs.map(l => ['runtime', l.timestamp, l.severity, l.sourcePackage, '',
